...
//	}
	...
	public boolean handleDeparture(double now, MobsimAgent agent, Id<Link> fromLinkId) {
		...
		PassengerRequest request = createRequest(passenger, fromLinkId, toLinkId, departureTime, now, agent.getMode());
		optimizer.requestSubmitted(request);
		//} else {
	...
	private PassengerRequest createRequest(MobsimPassengerAgent passenger, Id<Link> fromLinkId, Id<Link> toLinkId,
										   double departureTime, double now, String mode) {
		Map<Id<Link>, ? extends Link> links = network.getLinks();
		Link fromLink = links.get(fromLinkId);
		Link toLink = links.get(toLinkId);
		Id<Request> id = Id.create(this.mode + "_" + nextId++, Request.class);

		PassengerRequest request = requestCreator.createRequest(id, passenger, fromLink, toLink, departureTime, now, mode);
		return request;
	}
