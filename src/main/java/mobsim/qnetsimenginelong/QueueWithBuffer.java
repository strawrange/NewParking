...
final class QueueWithBuffer implements QLaneI, SignalizeableItem {
	...
	private double holeSpeed = Double.POSITIVE_INFINITY;
...
//		flowCapacityPerTimeStep = this.qLink.getLink().getFlowCapacityPerSec(now) ;
		...
		double flowCapacity_s = flowCapacityPerTimeStep * context.qsimConfig.getFlowCapFactor() ;
		flowCapacityPerTimeStep = flowCapacity_s * context.qsimConfig.getTimeStepSize();
		inverseFlowCapacityPerTimeStep = 1.0 / flowCapacityPerTimeStep;
		
		switch (context.qsimConfig.getTrafficDynamics()) {
			case queue:
			case withHoles:
				this.holeSpeed = HOLE_SPEED_KM_H/3.6;
				break;
			case kinematicWaves:
				...
				this.holeSpeed = 1/((1/(flowCapacity_s*context.effectiveCellSize))-(1/this.qLink.getFreespeed()));
				break;
...
//				} else if (handleTransitStop == HandleTransitStopResult.continue_driving) {
	...
	private void removeVehicleFromQueue(final QVehicle veh2Remove) {
		...
		switch (context.qsimConfig.getTrafficDynamics()) {
			...
			case kinematicWaves:
				Hole hole = new Hole() ;
				double ttimeOfHoles = length/holeSpeed ;
