...
final class QNodeImpl implements QNodeI {
	...
	private QNodeImpl(final Node n, NetsimEngineContext context, QNetsimEngine.NetsimInternalInterface netsimEngine2, TurnAcceptanceLogic turnAcceptanceLogic) {
		this.node = n;
		this.netsimEngine = netsimEngine2 ;
		this.context = context ;
		this.turnAcceptanceLogic = turnAcceptanceLogic;
	...
	private boolean moveVehicleOverNode(final QVehicle veh, final QLaneI fromLaneBuffer, final double now) {
		...
		TurnAcceptanceLogic.AcceptTurn turn = turnAcceptanceLogic.isAcceptingTurn(currentLink, fromLaneBuffer, nextLinkId, veh, this.netsimEngine.getNetsimNetwork());
		if ( turn.equals(TurnAcceptanceLogic.AcceptTurn.ABORT) ) {
			moveVehicleFromInlinkToAbort( veh, fromLaneBuffer, now, currentLink.getId() ) ;
			return true ;
		} else if ( turn.equals(TurnAcceptanceLogic.AcceptTurn.WAIT) ) {
			return false;
		}
